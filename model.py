...
def select_best_finished_beam(finished_sequences,finished_scores,alpha):
    ...
    for sequence,score in zip(finished_sequences,finished_scores):
        lp=compute_length_penalty(len(sequence),alpha)
        normalized_scores.append(score/lp)
    best_index=torch.tensor(normalized_scores).argmax().item()
    d={'sequence':finished_sequences[best_index],"score":normalized_scores[best_index]}
    return d
